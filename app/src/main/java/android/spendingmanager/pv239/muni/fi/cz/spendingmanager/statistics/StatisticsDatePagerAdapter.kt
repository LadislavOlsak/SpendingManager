...
class StatisticsDatePagerAdapter(fragmentManager: FragmentManager) : FragmentStatePagerAdapter(fragmentManager) {
    ...
    override fun getItem(position: Int): Fragment? {
        ...
        when (position) {
            0 -> {
                return StatisticsDateGraphs.newInstance(transactions as Serializable, categories as Serializable)
            }
            1 -> {
                return StatisticsDateGeneral.newInstance(transactions as Serializable, categories as Serializable)
            }
            else -> return null
        }
    }
